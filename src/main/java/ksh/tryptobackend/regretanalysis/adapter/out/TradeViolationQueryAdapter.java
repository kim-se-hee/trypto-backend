...
public class TradeViolationQueryAdapter implements TradeViolationQueryPort {

    private final AnalysisRuleQueryPort analysisRuleQueryPort;
    private final ViolationRecordQueryPort violationRecordQueryPort;
    private final TradeRecordQueryPort tradeRecordQueryPort;
    ...
    public List<TradeViolation> findByRoundIdAndExchangeId(Long roundId, Long exchangeId) {
        ...
        List<ViolationRecord> violations = violationRecordQueryPort
    ...
    private Map<Long, TradeRecord> loadOrderMap(List<ViolationRecord> violations) {
        List<Long> orderIds = violations.stream()
            .map(ViolationRecord::orderId)
            .filter(Objects::nonNull)
            .toList();
        return tradeRecordQueryPort.findByOrderIds(orderIds).stream()
        ...
        return buyOrderGroups.entrySet().stream()
            .collect(Collectors.toMap(
                ...
                entry -> {
                    ...
                    return tradeRecordQueryPort.findSellOrdersAfter(
                        entry.getKey().walletId(), entry.getKey().exchangeCoinId(), earliestFilledAt);
                }
            ));
    }

    private TradeViolation toTradeViolation(ViolationRecord violation,
